...
<c:if test="${findModel.getAllUserList() != null}">
	...
	<tr>
		<td><input type="checkbox" id="selectUser_${item.getUserId()}"/></td>
		...
		<td colspan="7" style="border:none;">
			<div style="float:right">
			   	<c:choose>
			   		...
			   		<c:otherwise>
			   			<input type="image" src="${pageContext.request.contextPath}/img/left_triangle.png" id="previousPage" style="width:30px;float:left" />
			   		</c:otherwise>
			   	</c:choose>
   				<select class="Select" name="currentPage" id="currentPage" style="width:60px;height:30px;float:left;margin:0px 5px 0px 5px;">
					...
					<c:set var="loopCount" value="${findModel.getAllUserList().size() / findModel.getShowNumber() + plus}" scope="request"/>
					<c:if test="${findModel.getShowNumber() != 0 && (findModel.getAllUserList().size() / findModel.getShowNumber() + 1) >= 1}">
				...
				<c:choose>
					<c:when test="${findModel.getCurrentPage() == loopCount}">
